...
export const HELP_SECTIONS = [
  ...
  },
  {
    ...
    body: [
      ...
      'Crosshairs (View menu): design a STABLE of overlay markers for the Lines area — combine shape, image and emoji layers with transparency, behind-the-glass distortion (blur, invert, hue, heat-haze warp) and ANIMATION (breathe, pulse, spin, rock, orbiting/scanning/oscillating dots, trippy fractals) — then place any of them on a tab and drag them where your eye should anchor. Animation periods are fixed seconds or paced to the current display line at your auto WPM (even with auto off). Designs are shared; placement and size are per tab.',
      'AVATARS (formerly faces, View → Avatar Library): they now act out what you\'re doing — bobbing while you scroll, nodding line-by-line, darting eyes word-by-word, sweeping on paragraph/page hops — mouth the words while the TTS speaks, get visibly drowsy as idle approaches, and SNOOZE with floating zzz and counting sheep once you go idle. Optional 🤚 floating hands — cartoon gloves tinted to match each avatar — float beside them and mirror your biometric controls (a detected 👍 flashes on a hand; the scroll joystick raises a gliding palm; claps applaud). On the typing screen the avatar\'s EYE COLOUR grades your live net WPM against consensus standards — gray beginner → green → cyan → blue → purple → gold → fire at 110+.',
      ...
      'Highlight algorithms (Tab Settings → Lines): research-backed word emphasis — fade function words (skilled readers barely fixate them), flag long words (fixation time grows with length), emphasize rare words (frequency is the strongest fixation predictor), or mark sentence starts (comprehension anchors). Style them bold, tinted, underlined or glowing; per tab, and they compose with the Bionic font and ORP marks.',
  ...
  },
  {
    ...
    body: [
      ...
      'Quitting a typing run is always safe: Esc mid-run ARMS first (press it again within 2.5s to actually end), and even Discard/Esc still counts every typed word as read — only the reading position stays put. Continue additionally moves you to the typed frontier.',
      ...
      '💥 Setback mode: mistakes ERASE progress — a mistyped word knocks you back 1, 3 or 5 committed words and you retype your way forward (a penalised word never ends the run, and your keystroke stats still stand). The harshest accuracy discipline in the drill.',
      '📋 Error log: a separate area listing every error entry as it happens — the target word vs what you actually typed, wrong and missed characters marked, corrected words included. Live during the run (in blind mode it waits for the end) and kept on the results screen.',
