...
| Backup & sync → Google Drive | Google Identity Services + Drive API | **Only if you turn it on.** Loads Google's sign-in script, signs you in, and syncs a **private app-data folder in your Drive** (only this app can see it). What travels: your reading progress, tab settings, and application settings — **never** your files, document text, or grabbed pages. Your browser talks directly to your Drive; nothing passes through any server of this app. On the official site no setup is needed; a self-hosted fork supplies its own OAuth client ID. Google's privacy policy applies. |
